...
class Truncation:
    ...
    def select_fittest_individuals(self) -> Set[Input]:
        ...
        logging.info(f"Total inputs: {len(sorted_inputs)}")
        ...
        logging.info(f"Truncation index: {trunc_index}")

        # Select fittest individuals
        selected_individuals = sorted_inputs[:trunc_index]
        logging.info(f"Selected individuals: {len(selected_individuals)}")

        fittest = set()
        if selected_individuals:
            for _ in range(self.population_size):
                r = random.randint(0, len(selected_individuals) - 1)
                logging.debug(f"Random index r: {r}")
                fittest.add(selected_individuals[r])
        else:
            logging.warning("No individuals selected due to empty selected_individuals list.")
    ...
    def select_fittest_individuals(self) -> Set[Input]:
        sorted_inputs = sorted(self.test_inputs, key=lambda inp: inp.fitness, reverse=True)
        trunc_index = max(1, int(len(sorted_inputs) * self.truncation_threshold))
        selected_individuals = sorted_inputs[:trunc_index]
